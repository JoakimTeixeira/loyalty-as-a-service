...
public class Loyaltycard {
	...
	public Uni<Boolean> save(MySQLPool client, Long idCustomer_R, Long idShop_R) {
		return client.preparedQuery("INSERT INTO LoyaltyCards(idCustomer,idShop) VALUES (?,?)")
	...
	public static Uni<Boolean> update(MySQLPool client, Long id_R, Long idCustomer_R, Long idShop_R) {
		return client.preparedQuery("UPDATE LoyaltyCards SET idCustomer = ? , idShop = ? WHERE id = ?")
				.execute(Tuple.of(idCustomer_R, idShop_R, id_R))
				.onItem().transform(pgRowSet -> pgRowSet.rowCount() == 1);
	}
}
