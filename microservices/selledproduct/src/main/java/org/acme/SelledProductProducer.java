...
public class SelledProductProducer {
    ...
    public Uni<Response> sendSelledProductMessage(String topicName, SelledProduct product) {
        ...
        return Uni.createFrom().completionStage(completionStage)
                .onItem().transform(response -> Response.ok(
                        Map.of(MESSAGE_KEY, "Selled Product product message sent: " + response)).build())
                .onFailure().recoverWithItem(error -> Response.serverError().entity(
                        Map.of(MESSAGE_KEY, "Failed to send selled product message")).build());
