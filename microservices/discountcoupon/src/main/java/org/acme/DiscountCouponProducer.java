...
@ApplicationScoped
public class DiscountCouponProducer {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DiscountCouponProducer.class);
    ...
    public Uni<Response> sendDiscountCouponMessage(String topicName, DiscountCoupon coupon) {
        CompletionStage<RecordMetadata> completionStage = CompletableFuture
                .supplyAsync(() -> {
                    try {
                        ...
                        throw new IllegalStateException("Failed to generate coupon", e.getCause());
                    } catch (InterruptedException e) {
                        Thread.currentThread().interrupt(); // Re-interrupt the thread
                        throw new IllegalStateException("Coupon generation interrupted", e);
                    }
                });
        ...
        return Uni.createFrom().completionStage(completionStage)
                .onItem().transform(response -> Response.ok(
                        Map.of(MESSAGE_KEY, "Discount coupon message sent: " + response)).build())
                .onFailure().recoverWithItem(error -> Response.serverError().entity(
                        Map.of(MESSAGE_KEY, "Failed to send discount coupon message")).build());

    }

    public Future<RecordMetadata> generateCoupon(String topicName, Long id, String couponContent) {
        String couponKey = "Coupon-" + id;
        ProducerRecord<String, String> createdCoupon = new ProducerRecord<>(topicName, couponKey, couponContent);

        return producer.send(createdCoupon, (metadata, exception) -> {
            if (exception == null) {
                LOG.info("Successfully sent message to topic {}: partition {}, offset {}",
                        metadata.topic(), metadata.partition(), metadata.offset());
            } else {
                LOG.error("Failed to send message to topic {}", createdCoupon.topic(), exception);
            }
        });
    }
}
